...
class GatewayManager {
        ...
        let myPromise = new Promise(function(myResolve, myReject) {
            gwDevices.list(lstGWReq, metadata, (err, answer) => {
                if(err){                    
                    ...
                    console.log("Gateway:" + gatewayInfo);
    ...
    queryServer(lstGWReq){
        let myPromise = new Promise(function(myResolve, myReject) {
            gwDevices.list(lstGWReq, this.metadata, (err, answer) => {
                if(err){                    
                    myReject(err)
                } else {
                    let gatewayInfo = {};
...
let myPromise = gwManager.initialize();
let gwList = await myPromise.then(
    function(value) {return value;},
    function(error) {return {};});
console.log(JSON.stringify(gwList));
